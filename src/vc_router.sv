...
//   independent VC FIFOs instead of one buffer. Arriving packets are spread
...
//   output         : unchanged depth-1 valid_ready_slice; packet goes back onto
//                    the plain 8-bit wire (the VC identity is the plane index,
//                    so there is no literal tag to strip in phase 1).
//
//   The packet on every inter-router wire stays 8 bits, so torus_4x4 and the
...
// Valid_ready_slice ( used at output ports )
