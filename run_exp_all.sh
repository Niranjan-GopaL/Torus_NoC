...
cleanup() {
    ...
    bash "$SRC_DIR/switch_routing.sh"  custom        >/dev/null || true
    bash "$SRC_DIR/switch_workload.sh" UNIFORM_RANDOM 70 10 >/dev/null || true
...
run_one() {
    ...
    bash "$SRC_DIR/switch_routing.sh" "${ALGO,,}" >/dev/null

    local CSV_ROW
    CSV_ROW=$(bash "$SRC_DIR/run_one_workload.sh" "$ALGO" "$PATTERN" "$BP_READY" "$BP_HOT" "$FIFO")
...
for PAT in "${PATTERNS[@]}"; do
    ...
    for i in 0 1 2; do
        ...
        ROW=$(bash "$SRC_DIR/switch_routing.sh" "${ALGO_TAGS[$i]}" >/dev/null && \
              bash "$SRC_DIR/run_one_workload.sh" "$ALGO" "$PAT" 70 10 8)
        echo "  -> $ROW"
...
for HOT in "${HOTSPOT_PCTS[@]}"; do
    ...
    for i in 0 1 2; do
        ALGO="${ALGOS[$i]}"
        echo ""
        echo "## Exp3: $ALGO routing, HOTSPOT $HOT%"
        ROW=$(bash "$SRC_DIR/switch_routing.sh" "${ALGO_TAGS[$i]}" >/dev/null && \
              bash "$SRC_DIR/run_one_workload.sh" "$ALGO" HOTSPOT 70 "$HOT" 8)
        echo "  -> $ROW"
...
for BP in "${BP_READY_PCTS[@]}"; do
    ...
    for i in 0 1 2; do
        ...
        ROW=$(bash "$SRC_DIR/switch_routing.sh" "${ALGO_TAGS[$i]}" >/dev/null && \
              bash "$SRC_DIR/run_one_workload.sh" "$ALGO" UNIFORM_RANDOM "$BP" 10 8)
        echo "  -> $ROW"
...
for PAT in "${ADV_PATTERNS[@]}"; do
    for FIFO in "${FIFO_DEPTHS[@]}"; do
        ...
        for i in 0 1 2; do
            ...
            ROW=$(bash "$SRC_DIR/switch_routing.sh" "${ALGO_TAGS[$i]}" >/dev/null && \
                  bash "$SRC_DIR/run_one_workload.sh" "$ALGO" "$PAT" 70 10 "$FIFO")
            echo "  -> $ROW"
