...
// module tb_mesh_4x4_congestion_b2b;
...
//     initial begin
//         $dumpfile("mesh_4x4_congestion_b2b.vcd");
//         $dumpvars(0, tb_mesh_4x4_congestion_b2b);
//     end
...
module tb_torus_4x4_congestion_b2b;
    ...
    // Modified to match torus_topology interface (unpacked arrays)
    logic [15:0]       local_in_valid;
    logic [7:0]        local_in_data   [0:15];
    logic [15:0]       local_in_ready;

    logic [15:0]       local_out_valid;
    logic [7:0]        local_out_data  [0:15];
    logic [15:0]       local_out_ready;
    ...
    // Changed from mesh_4x4 to torus_topology
    torus_topology dut (
        .clk            (clk),
        .rst_n          (rst_n),
    ...
    task automatic send_b2b_from_src(
        ...
        begin
            ...
            for (int p = 0; p < NUM_PKTS_PS; p++) begin
                ...
                // Modified to use unpacked array access
                local_in_valid      |= (1 << src_id);
                local_in_data[src_id] = pkt;

                @(posedge clk);

                while (!local_in_ready[src_id]) begin
                    @(posedge clk);
                end
    ...
    initial begin
        ...
        local_in_valid  = '0;
        // local_in_data already initialized to '0 via array initialization
        for (int i = 0; i < 16; i++) begin
            local_in_data[i] = '0;
        end
        local_out_ready = '1;
        ...
        for (int t = 0; (t < 20000) && (recv_count < EXP_PKTS); t++) begin
            ...
            // No packet should appear at any wrong local destination
            for (int d = 0; d < 15; d++) begin
                if (local_out_valid[d] && local_out_ready[d]) begin
                    $display("FAIL: wrong destination dst=%0d pkt=0x%0h time=%0t",
                             d, local_out_data[d], $time);
                    fail_count++;
                end
            ...
            // Expected destination = node 15
            if (local_out_valid[DST_ID] && local_out_ready[DST_ID]) begin
                logic [3:0] payload;
