...
// =============================================================================
// vc_router.sv — PHASE 1 virtual-channel router (VC plumbing only)
//
// WHAT THIS IS
//   A drop-in for the previous router. Each physical input port now has NUM_VC
//   independent VC FIFOs instead of one buffer. Arriving flits are spread
//   across VCs by a parameterized round-robin. Routing is UNCHANGED (your
//   proven 4x4 torus tie-split), so deadlock-freedom still rests entirely on
//   the routing function — this phase only adds the VC buffering + the
//   two-stage allocator. VC-in == VC-out (no dateline yet; that is phase 2).
//
// ARCHITECTURE
//   per input port : NUM_VC * fifo_sync  + a round-robin VC-assign counter
//   "plane" v      : a full copy of the proven 5-split / 5-merge network,
//                    fed by the VC-v FIFO of every port. NUM_VC planes total.
//   stage-2 arb    : per output port, one merge_4to1_comb(NUM_PORTS=NUM_VC)
//                    picks among the NUM_VC planes' winners. (Your masked RR,
//                    reused at a narrower width.)
//   output         : unchanged depth-1 valid_ready_slice; flit goes back onto
//                    the plain 8-bit wire (the VC identity is the plane index,
//                    so there is no literal tag to strip in phase 1).
//
//   The flit on every inter-router wire stays 8 bits, so torus_4x4 and the
//   testbench are untouched.
//
...
// =============================================================================
// 1. xy_route_logic — 4x4 torus tie-split routing (UNCHANGED, deadlock-free)
// =============================================================================
...
// =============================================================================
// 2. valid_ready_slice — skid buffer (UNCHANGED, used on outputs)
// =============================================================================
...
// =============================================================================
// 2b. fifo_sync — synchronous FIFO (UNCHANGED from the FIFO router)
//   ready_out = !full (occupancy-only), valid_out = !empty, registered head.
//   extra-MSB full/empty trick, correct for any DEPTH >= 2.
// =============================================================================
module fifo_sync #(
...
// =============================================================================
// 3. split_1to4_simple — 1->4 demux with routing (UNCHANGED)
// =============================================================================
...
module router_plane (
    ...
    // upstream (from this plane's per-port VC FIFOs)
    input  logic [4:0]  pin_valid,
    input  logic [39:0] pin_data,
    output logic [4:0]  pin_ready,

    // downstream (to stage-2 per-output VC arbiter)
...
// =============================================================================
// 5. vc_router — top router with VC input buffering + two-stage arbitration
// =============================================================================
module vc_router #(
    parameter int NUM_VC     = 2,   // >= 2
    ...
    // ---- per-port VC FIFOs + round-robin assignment ----
    ...
    // ---- stage-2: per output direction, arbitrate across VCs ----
    logic [7:0] merge_to_slice_data  [0:4];
    ...
    generate
        for (gd = 0; gd < 5; gd = gd + 1) begin : g_stage2
            ...
            merge_4to1_comb #(.DATA_WIDTH(8), .NUM_PORTS(NUM_VC)) u_stage2 (
                ...
                .ready_in (s2_ready),
                .data_in  (s2_data),
                .valid_out(merge_to_slice_valid[gd]),
                .ready_out(slice_to_merge_ready[gd]),
                .data_out (merge_to_slice_data[gd])
            );
        end
    endgenerate

    // ---- output slices (UNCHANGED) ----
    genvar go;
    generate
...
// =============================================================================
// 6. torus_router_5x5 — wrapper (UNCHANGED interface; forwards NUM_VC/DEPTH)
// =============================================================================
