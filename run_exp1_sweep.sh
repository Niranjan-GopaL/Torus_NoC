...
RESULTS_FILE="$SRC_DIR/results.md"
...
for DEPTH in "${FIFO_DEPTHS[@]}"; do
    ...
    cp "$DEPTH_FILE.orig" "$DEPTH_FILE"
    sed -i "s/parameter int FIFO_DEPTH = 64/parameter int FIFO_DEPTH = $DEPTH/" "$DEPTH_FILE"
    ...
    # TCL for this run
    TCL_FILE="/tmp/exp1_run_${ALGO_LABEL}_${DEPTH}.tcl"
    LOG_FILE="/tmp/exp1_run_${ALGO_LABEL}_${DEPTH}.log"
